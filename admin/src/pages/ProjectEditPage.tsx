...
import { ProjectMediaManager } from "../components/ProjectMediaManager.js";
...
export function ProjectEditPage() {
  ...
  const loadProject = async (projectId: string) => {
    try {
      ...
      const draft = getProjectDraft(getEditProjectDraftKey(projectId));
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      clearProjectDraft(getEditProjectDraftKey(id));
      navigate("/projects", {
        state: { message: `Project "${project.title}" was permanently deleted.` },
      });
    } catch (err) {
