...
import { ProjectsListPage } from "../src/pages/ProjectsListPage.js";
import { ProjectCreatePage } from "../src/pages/ProjectCreatePage.js";
import { ProjectEditPage } from "../src/pages/ProjectEditPage.js";
import { ProjectPreviewPage } from "../src/pages/ProjectPreviewPage.js";
import { EquipmentEditor } from "../src/components/EquipmentEditor.js";
import { ImageSlotManager } from "../src/components/ImageSlotManager.js";
import { ProjectMediaManager } from "../src/components/ProjectMediaManager.js";
import { DeleteConfirmModal } from "../src/components/DeleteConfirmModal.js";
import { ApiError } from "../src/lib/api.js";
import type { AdminProject, ProjectImageItem } from "../src/types/project.js";
...
describe("Phase 4B: Projects Management UI", () => {
  ...
  // 4. Create Project: Sections 1 & 2 only, NO media section, redirects to /projects/:id/edit
  it("4. ProjectCreatePage contains only sections 1 & 2, omits media controls, and redirects to edit page", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/projects/new"]}>
        <Routes>
          ...
          <Route path="/projects/:id/edit" element={<div>Project Editor Loaded</div>} />
        </Routes>
      </MemoryRouter>,
    );
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("Project Editor Loaded")).toBeDefined();
    });
  });
  ...
  it("7. ProjectEditPage handles 400 validation error when primary image is missing", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/projects/proj-2/edit"]}>
        <Routes>
          <Route path="/projects/:id/edit" element={<ProjectEditPage />} />
        </Routes>
      </MemoryRouter>,
  ...
  it("9. ProjectPreviewPage renders simulated card and toggles expand/collapse", async () => {
    ...
    render(
      <MemoryRouter initialEntries={["/projects/proj-1/preview"]}>
        <Routes>
  ...
  it("12. ProjectMediaManager enforces remaining capacity and processes multi-image queue", async () => {
    const user = userEvent.setup();
    const onMediaChange = vi.fn();
    const { uploadAdminProjectMedia } = await import("../src/lib/api.js");
  ...
  describe("Homepage Featured Projects Controls", () => {
    ...
    it("ProjectEditPage: featuring first published project sends [project.id] UUID and never slug, refetches project state, and updates UI", async () => {
      ...
      render(
        <MemoryRouter initialEntries={[`/projects/${testUuid}/edit`]}>
          <Routes>
            <Route path="/projects/:id/edit" element={<ProjectEditPage />} />
          </Routes>
        </MemoryRouter>,
